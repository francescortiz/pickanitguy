...
export const makeDebugDraw = (
	 ...
	 * @returns {void}
	 */
	const setCtxColor = (rgbStr: string) => {
	 ...
	 * @returns {void}
	 */
	const drawPolygon = (vertices: Array<Box2D.b2Vec2>, vertexCount: number, fill: boolean) => {
		ctx.beginPath();
		let first = true;
	 ...
	 * @returns {void}
	 */
	const drawSegment = (vert1: Box2D.b2Vec2, vert2: Box2D.b2Vec2) => {
		ctx.beginPath();
	 ...
	 * @returns {void}
	 */
	const drawPoint = (vertex: Box2D.b2Vec2, sizeMetres: number) => {
	 ...
	 * @returns {void}
	 */
	const drawTransform = (transform: Box2D.b2Transform, sizeMetres?: number) => {
	...
	const debugDraw = Object.assign(new JSDraw(), {
		 ...
		 * @returns {void}
		 */
		DrawSegment(vert1_p: number, vert2_p: number, color_p: number) {
		 ...
		 * @returns {void}
		 */
		DrawPolygon(vertices_p: number, vertexCount: number, color_p: number) {
			const color = wrapPointer(color_p, b2Color);
			setCtxColor(getRgbStr(color));
		 ...
		 * @returns {void}
		 */
		DrawSolidPolygon(vertices_p: number, vertexCount: number, color_p: number) {
			const color = wrapPointer(color_p, b2Color);
			setCtxColor(getRgbStr(color));
			const vertices = reifyArray(vertices_p, vertexCount, sizeOfB2Vec, b2Vec2);
		 ...
		 * @returns {void}
		 */
		DrawCircle(center_p: number, radius: number, color_p: number) {
			const color = wrapPointer(color_p, b2Color);
			setCtxColor(getRgbStr(color));
		 ...
		 * @returns {void}
		 */
		DrawSolidCircle(center_p: number, radius: number, axis_p: number, color_p: number) {
			const color = wrapPointer(color_p, b2Color);
			setCtxColor(getRgbStr(color));
			const center = wrapPointer(center_p, b2Vec2);
		 ...
		 * @returns {void}
		 */
		DrawTransform(transform_p: number) {
		 ...
		 * @returns {void}
		 */
		DrawPoint(vertex_p: number, sizeMetres: number, color_p: number) {
			const color = wrapPointer(color_p, b2Color);
			setCtxColor(getRgbStr(color));
